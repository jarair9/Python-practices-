...
if not os.path.exists(api_key):
    raise Exception(Fore.RED +"API key is not found" + Fore.RESET)
genai.configure(api_key=api_key)
model = genai.GenerativeModel(model_name="models/gemini-2.0-flash")

lvl = str(input(Fore.GREEN + "Enter Your Level  beginner , Intermiate , Advance : " + Fore.RESET))

# Prompt
response = model.generate_content(f"""
        You are a Python expert. Please generate Python quiz questions accordings to user skill and level 'User level {lvl}', each with multiple-choice options and the correct answer.
...
print(f"Time taken for Quiz: {elapsed:.2f} seconds")
